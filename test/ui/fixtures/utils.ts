...
// Waits until an editor titled `title` is open and returns it. `openEditor`
// rejects until the tab exists, so retry via driver.wait rather than a fixed
// sleep (the editor may still be opening when this is called).
export function waitForEditor(title: string): Promise<Editor> {
  ...
  return VSBrowser.instance.driver.wait(
    () => editors.openEditor(title).catch(() => undefined),
    5000,
    title,
  );
}
