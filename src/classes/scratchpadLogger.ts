...
export class ScratchpadLogger {
  ...
  constructor(private readonly connection: InsightDetails) {
  ...
  private processLogs(data: LogMessage["data"]) {
    ...
    data.forEach(({ handle, value }) => {
      const decoded = decodeQUTF(value);
      if (handle === "STDERR") {
        queryConsole.appendStdErr(decoded);
      } else if (handle === "STDOUT") {
        queryConsole.appendStdOut(decoded);
      }
    });
  }
