...
export function parseUDAParams(
  ...
  params.forEach((param) => {
    ...
    // WHEN SERVICEGATEWAY ACCEPT MULTI-TYPES, REMOVE THE MULTI-TYPE PART
    if (
      (fieldType === ParamFieldType.Invalid && param.isReq) ||
      (fieldType === ParamFieldType.MultiType && param.isReq)
    ) {
