...
export default class QBoolean extends U8 {
  static readonly typeNum = 1;
  ...
  static listToIPC(values: Array<boolean>): Uint8Array {
    ...
    buffer.wb(QBoolean.typeNum);
    buffer.wb(0);
    buffer.wi(values.length);
    values.forEach((v) => buffer.wb(v ? 1 : 0));
  ...
  static toIPC(value: boolean): Uint8Array {
    ...
    buffer.wb(256 - QBoolean.typeNum);
    buffer.wb(value ? 1 : 0);
    return buffer.data;
  }
